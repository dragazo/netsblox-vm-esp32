...
fn main() {
    ...
    let img = zune_image::image::Image::open_from_mem(include_bytes!("../test-img.png"), Default::default()).unwrap();
    println!("{:?}", img.get_metadata());

    // println!("here 1");
    // let data = image::load_from_memory(include_bytes!("../test-img.png")).unwrap();
    // println!("here 2");
